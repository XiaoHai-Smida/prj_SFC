...
avged = pd.DataFrame()
avged['a'] = (data['b1']+data['b4'])/2
avged['b'] = (data['b2']+data['b5'])/2
avged['c'] = (data['b3']+data['b6'])/2
## add envs
X = pd.concat([X, data.iloc[:,0], avged],axis=1)
y = data.iloc[:,7:10]
'''

X = pd.read_csv("E:\\Users\\admin\\Desktop\\isv.csv")
y = pd.read_csv("E:\\Users\\admin\\Desktop\\isv_behave.csv")
...
# model parameters
num_samples, num_features = scaled_X.shape
batch_size = 32
num_outputs = 3
num_epoch = 500
mse_ratio = 1
...
for cv in range(num_samples):
    ...
    # model definition
    # model = JointModel(input_dim=num_features, output_dim=num_outputs).to(device)
    model = RegressionNet(input_dim=num_features, output_dim=num_outputs).to(device)
...
r[0] = pearson_correlation(final_predictions[:,0], scaled_y[:,0])
r[1] = pearson_correlation(final_predictions[:,1], scaled_y[:,1])
r[2] = pearson_correlation(final_predictions[:,2], scaled_y[:,2])

print(f'pearson r:{r}\n')

plt.scatter(scaled_y[:,0], final_predictions[:, 0], c='blue')
plt.scatter(scaled_y[:,1], final_predictions[:, 1], c='red')
plt.scatter(scaled_y[:,2], final_predictions[:, 2], c='green')
plt.show()
